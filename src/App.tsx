...
function CubeLoader(props: {counter: number, data: Solve, time: number}) {
  ...
  const offset = time - data.starting_time
  let seq = React.useMemo( () => {
    ...
    for (let step of data.stages) {
      ...
      const pause = Math.min(250, time * 0.3)
